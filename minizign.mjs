...
export class Minizign {
  ...
  publicKey (base64String) {
    ...
    const { publicKeyDecodeFromBase64 } = this.instance.exports

    const encodedString = this.encode(base64String)

    const resultFromBase64 = publicKeyDecodeFromBase64(encodedString.index, encodedString.length)
...
export class Signature {
  ...
  getTrustedComment () {
    ...
    const { memory, signatureGetTrustedComment, signatureGetTrustedCommentLength } = instance.exports

    const index = signatureGetTrustedComment(this.index)
    const length = signatureGetTrustedCommentLength(this.index)
